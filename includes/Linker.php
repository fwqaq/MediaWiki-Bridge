...
class Linker {
	...
	function makeLinkObj( $nt, $text= '', $query = '', $trail = '', $prefix = '' ) {
		...
		if ( $nt->isExternal() ) {
			...
			// MindTouch (steveb): use special encoding for interwiki links
			if($nt->getFragment() !== '') {
				$t = '<mediawiki:interwiki prefix="'.htmlspecialchars($nt->getInterwiki()).'" path="'.htmlspecialchars($nt->getDBkey()).'" fragment="'.htmlspecialchars($nt->getFragment()).'">'.$text.$inside.'</mediawiki:interwiki>';
			} else {
				$t = '<mediawiki:interwiki prefix="'.htmlspecialchars($nt->getInterwiki()).'" path="'.htmlspecialchars($nt->getDBkey()).'">'.$text.$inside.'</mediawiki:interwiki>';
			}
			// MindTouch: --- end
	...
	function makeKnownLinkObj( $nt, $text = '', $query = '', $trail = '', $prefix = '' , $aprops = '', $style = '' ) {
		...
		// MindTouch (steveb): use special encoding for well-known links
		$inner = $prefix . $text . $inside;
		if($nt->getInterwiki() != '') {
			if($u == $inner) {
				$r = "<mediawiki:interwiki prefix=\"{htmlspecialchars($nt->getInterwiki())}\" path=\"{htmlspecialchars($nt->getDBkey())}\"{$aprops} />{$trail}";
			} else {
				$r = "<mediawiki:interwiki prefix=\"{htmlspecialchars($nt->getInterwiki())}\" path=\"{htmlspecialchars($nt->getDBkey())}\"{$aprops}>{$inner}</mediawiki:interwiki>/{$trail}";
			}
		} else {
			if($u == $inner) {
				$r = "<mediawiki:link type=\"internal\" ns=\"{$nt->getNamespace()}\" href=\"{$u}\"{$aprops} />{$trail}";
			} else {
				$r = "<mediawiki:link type=\"internal\" ns=\"{$nt->getNamespace()}\" href=\"{$u}\"{$aprops}>{$inner}</mediawiki:link>{$trail}";
			}
		}
		// MindTouch: --- end
	...
	function makeExternalImage( $url, $alt = '' ) {
		...
		// MindTouch (steveb): use special encoding for external images
		if ( '' == $alt ) {
			return '<mediawiki:image type="external" href="' . $url . '" alt="' . $alt . '" />';
		}
		return '<mediawiki:image type="external" href="' . $url . '" />';
		// MindTouch: --- end
	...
	function makeImageLink2( Title $title, $file, $frameParams = array(), $handlerParams = array() ) {
		...
		// MindTouch (steveb): use special encoding for internal images
		$parts = split(':', $title->getLocalURL(), 2);
		$result = '<mediawiki:image type="internal" href="' . htmlspecialchars($parts[1]) . '" ';
		if(isset($fp['alt']) && ($fp['alt'] != '')) {
			$result .= 'alt="' . $fp['alt'] . '" ';
		}
		if(isset($fp['align']) && ($fp['align'] != '')) {
			$result .= 'align="' . $fp['align'] . '" ';
		}
		if(isset($fp['valign']) && ($fp['valign'] != '')) {
			$result .= 'valign="' . $fp['valign'] . '" ';
		}
		if(isset($hp['width']) && ($hp['width'] != '')) {
			$result .= 'width="' . $hp['width'] . '" ';
		}
		if(isset($hp['height']) && ($hp['height'] != '')) {
			$result .= 'height="' . $hp['height'] . '" ';
		}
		
		// TODO (steveb): add desired image style
	...
	function makeMediaLinkObj( $title, $text = '' ) {
		if( is_null( $title ) ) {
			...
			// MindTouch (steveb): use special encoding for media links
			$parts = split(':', $title->escapeLocalURL(), 2);
			$u = $parts[1];
			return '<mediawiki:link type="file" href="'.$u.'" alt="'.$alt.'">'.$text.'</mediawiki:link>';
			// MindTouch: --- end
	...
	function makeExternalLink( $url, $text, $escape = true, $linktype = '', $ns = null ) {
		...
		// MindTouch (steveb): use special encoding for external links
		if($linktype == 'free') {
			return '<mediawiki:link type="external-free" href="'.$url.'" />';
		} else if($linktype == 'autonumber') {
			return '<mediawiki:link type="external-ref" href="'.$url.'" />';
		} else if(strpos($text, "<mediawiki:") === false) {
			return '<mediawiki:link type="external" href="'.$url.'">'.$text.'</mediawiki:link>';
		} else {
		
			// link contains active elements, keep it in its original form
			return '['.$url.$text.']';
		}
		// MindTouch: --- end
	...
	public function makeHeadline( $level, $attribs, $anchor, $text, $link ) {
		...
		// MindTouch (steveb): keep headline simple
		return "<h".($level+1)." name=\"{$anchor}\"$attribs$link$text</h".($level+1).">";
		// MindTouch: --- end
