		...
		foreach( $matches as $marker => $data ) {
			...
			if( $render ) {
				...
				case '!--':
					...
					// MindTouch (steveb): preserve 'comment' tag
					$element = '';
					$output = '<mediawiki:comment>' . htmlspecialchars(trim(strtr(trim(substr($output, 2, -1), "!- "), array( "\n" => ' ', "\r" => '')))) . '</mediawiki:comment>';
					// MindTouch: --- end
				...
				case 'nowiki':
					...
					// MindTouch (steveb): preserve 'nowiki' tag
					$output = '<mediawiki:nowiki>' . Xml::escapeTagsOnly( $content ) . '</mediawiki:nowiki>';
					// MindTouch: --- end
					
					break;
					
				// MindTouch (steveb): disable 'math' tag (use tag hooks instead)
				// case 'math':
				...
				default:
					if( isset( $this->mTagHooks[$tagName] ) ) {
						...
						// MindTouch (steveb): emit custom tag hooks
						if($this->mTagHooks[$tagName] === '') {
							$output = '<mediawiki:extension function="' . htmlspecialchars($tagName) . '"';
							if($params != null) {
...
'<head><title>test</title></head><body>'.$text.'</body></html>';
	...
	function internalParse( $text ) {
		...
		// MindTouch (steveb): disable removal of <includeonly>, <onlyinclude>, and <noinclude> elements
		//$text = strtr( $text, array( '<onlyinclude>' => '' , '</onlyinclude>' => '' ) );
		//$text = strtr( $text, array( '<noinclude>' => '', '</noinclude>' => '') );
		//$text = StringUtils::delimiterReplace( '<includeonly>', '</includeonly>', '', $text );
		$text = strtr( $text, array( '<onlyinclude>' => '<div class="onlyinclude">', '</onlyinclude>' => '</div>') );
		$text = strtr( $text, array( '<noinclude>' => '<div class="noinclude">', '</noinclude>' => '</div>') );
		$text = strtr( $text, array( '<includeonly>' => '<div class="includeonly">', '</includeonly>' => '</div>') );
		// MindTouch: --- end
	...
	function replaceInternalLinks( $s ) {
		...
		for ($k = 0; isset( $a[$k] ); $k++) {
			...
			# Self-link checking
			if( $nt->getFragment() === '' ) {
				// MindTouch (steveb): don't proces self-links; they are context-sensitive
				...
				if ( $openmatch or $closematch ) {
					...
					if ( ' ' == $t{0} and ( $this->mLastSection == 'pre' or trim($t) != '' ) ) {
						...
						if ($this->mLastSection != 'pre') {
							...
							// MindTouch (steveb): create <pre> element depending if it was intended to a <pre> or started with an empty space
							if(' ' == $t{0}) {
								$output .= $this->closeParagraph().'<pre class="eval">';
							} else {
	...
	function findColonNoLinks($str, &$before, &$after) {
		...
		// MindTouch (steveb): comment out search for ':' as it messes up our custom logic
		/*
		$lt = strpos( $str, '<' );
		if( $lt === false || $lt > $pos ) {
			...
			case 0: // MW_COLON_STATE_TEXT:
				...
				case ":":
					if( $stack == 0 ) {
						...
						// MindTouch (steveb): don't accept ':' if it's prefixed by a letter and followed by "//"
						$prefix = substr($before, -1);
						if((($prefix >= "a" && $prefix <= "z") || ($prefix >= "A" && $prefix <= "Z")) && substr($after, 0, 2) === "//") {
						
							// not a good place to break
							break;
						}
						// MindTouch: --- end
	...
	function braceSubstitution( $piece ) {
		...
		// MindTouch (steveb): generate template code
		global $wgContLang;
		$title = $piece['title'];
		...
		if (null == $piece['parts']) {
			if(array_key_exists($wgContLang->lc($piece['title']), $this->mVariables)) {
				return '<mediawiki:magic name="' . $piece['title'] . '" />';
			}
		}
		...
		if( $colonPos !== false ) {
			if( $title{0} === '#' ) {
				$function = substr( $title, 0, $colonPos );
			} else {
				$function = substr( $title, 0, $colonPos );
		...
		if ( $function ) {
			if( $function{0} === '#' ) {
				...
				// generate <mediawiki:expression> tags
				$result = '<mediawiki:expression name="' . htmlspecialchars($function) . '">';
				$result .= '<mediawiki:arg>' . trim(substr($title, $colonPos+1)) . '</mediawiki:arg>';
				foreach($args as $key => $value) {
					$result .= '<mediawiki:arg>' . $value . '</mediawiki:arg>';
				}
				$result .= '</mediawiki:expression>';
			} else {
		
				// generate <mediawiki:function> tags
				$result = '<mediawiki:function name="' . htmlspecialchars($function) . '">';
				$result .= '<mediawiki:arg>' . trim(substr($title, $colonPos+1)) . '</mediawiki:arg>';
				foreach($args as $key => $value) {
					$result .= '<mediawiki:arg>' . $value . '</mediawiki:arg>';
				}
				$result .= '</mediawiki:function>';
			}
		} else {
		
			// generate <mediawiki:template> tags
			if($title == "!") {
				$result .= '|';
			} else {
				$canonicalTitle = Title::newFromDBkey($title);
				if($canonicalTitle) {
					if($title{0} == ':') {
						$result = '<mediawiki:template><mediawiki:name>:' . $canonicalTitle->getNormalizedDbKey() . '</mediawiki:name>';
					} else {
						$result = '<mediawiki:template><mediawiki:name>' . $canonicalTitle->getNormalizedDbKey() . '</mediawiki:name>';
					}
				} else {
					$result = '<mediawiki:template><mediawiki:name>' . $title . '</mediawiki:name>';
				}
				foreach($args as $key => $value) {
					$result .= '<mediawiki:arg>' . $value . '</mediawiki:arg>';
				}
				$result .= '</mediawiki:template>';
			}
		}
		return $result;
		// MindTouch: --- end
	...
	function argSubstitution( $matches ) {
		...
		$result = '';
		if(intval($arg) > 0) {
			if(null != $matches['parts'] && count($matches['parts']) > 0) {
				$result .= '<mediawiki:ref index="' . $arg . '" alt="true">';
				$result .= $matches['parts'][0];
				$result .= '</mediawiki:ref>';
			} else {
				$result .= '<mediawiki:ref index="' . $arg . '" />';
			}
		} else {
			if(null != $matches['parts'] && count($matches['parts']) > 0) {
				$result .= '<mediawiki:ref name="' .  htmlspecialchars($arg) . '" alt="true">';
				$result .= $matches['parts'][0];
				$result .= '</mediawiki:ref>';
			} else {
				$result .= '<mediawiki:ref name="' .  htmlspecialchars($arg) . '" />';
			}
		}
		return $result;
		// MindTouch: --- end
	...
	function formatHeadings( $text, $isMain=true ) {
		...
		// MindTouch (steveb): never show edit links
		$showEditLink = 0;
		// MindTouch: --- end
		...
		// MindTouch (steveb): make limit for TOC to appear automatically configurable (default is 4)
		global $wgRequest;
		$minNumMatches = intval($wgRequest->getVal( 'autotoc', '4' ));
		if($minNumMatches <= 0) {
			$minNumMatches = PHP_INT_MAX;
		}
		// MindTouch: --- end
		...
		$enoughToc = $this->mShowToc &&
			(($numMatches >= $minNumMatches) || $this->mForceTocPosition);
		...
		if( $enoughToc ) {
			...
			// MindTouch (steveb): use dekiscript to insert toc; disbled for now
			// $toc = '<mediawiki:extension value="page.toc" />';
			$toc = '';
			// MindTouch: --- end
	...
	function replaceLinkHolders( &$text, $options = 0 ) {
		...
		if ( !empty( $this->mLinkHolders['namespaces'] ) ) {
			...
			foreach ( $this->mLinkHolders['namespaces'] as $key => $ns ) {
				...
				// MindTouch (steveb): always show links as known
				$colours[$pdbk] = 1;
				continue;
				// MindTouch: --- end
	...
	function attributeStripCallback( &$text, $args ) {
		...
		// MindTouch (steveb): attributes require special care
		$text = preg_replace_callback(
			'/(\w+)="((?:\\.|[^"])*)"/',
			array( &$this, 'attributeStripCallbackCallback' ),
			$text
		);
		// MindTouch: --- end
...
//		$text = $this->replaceVariables( $text, $args );
	...
	// MindTouch (steveb): attributes require special care
	function attributeStripCallbackCallback($matches) {
		$key = $matches[1];
		$originalValue = $matches[2];
		$value = $this->replaceVariables($originalValue, array());
		if($value == $originalValue) {
			$text = $key.'="'.$value.'"';
		} else {
			$value = strtr($value, array('<' => '.26lt;', '>' => '.26gt;', '&' => '.26amp;', '"' => '.26quot;'));
			$text = 'mediawiki:'.$key.'="'.$value.'"';
		}
		return $text;
	}
	// MindTouch: --- end
