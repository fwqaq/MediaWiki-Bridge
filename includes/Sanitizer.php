...
// MindTouch (steveb): original attribute definition didn't allow for namespaced attributes
$attrib = '[A-Za-z0-9]+(?::[A-Za-z0-9]+)?';
...
class Sanitizer {
	...
	static function removeHTMLtags( $text, $processCallback = null, $args = array(), $extratags = array() ) {
		...
		if ( !$staticInitialised ) {
			...
			$htmlpairs = array_merge( $extratags, array( # Tags that must be closed
				...
				// MindTouch (steveb): add <noinclude> to set of valid tags
				'noinclude',
				// MindTouch: --- end
	...
	static function validateAttributes( $attribs, $whitelist ) {
		...
		foreach( $attribs as $attribute => $value ) {
			...
			// MindTouch (steveb): always keep attributes prefixed by 'mediawiki:'
			if(strpos($attribute, "mediawiki:") === 0) {
				$out[$attribute] = $value;
				continue;
			}
			// MindTouch: --- end
