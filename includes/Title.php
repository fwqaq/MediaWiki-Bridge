...
class Title {
	...
	// MindTouch (steveb): add key to interwiki cache
	public static function AddCachedPrefix($key) {
...
#----------------------------------------------------------------------------
	...
	// MindTouch (steveb): return normalized db-key
	public function getNormalizedDbKey() {
		return Title::makeName($this->getNamespace(), $this->mTextform);
	}
	// MindTouch: --- end
	...
	public function getInterwikiLink( $key )  {
		...
		// MindTouch (steveb): check if key exists in imported interwiki list
		global $wgInterwikiList;
		if( array_key_exists( $key, $wgInterwikiList ) ) {
			return 'interwiki://' . $wgInterwikiList[$key] . '/';
		}		
		// MindTouch: --- end
